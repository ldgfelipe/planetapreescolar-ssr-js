<template>
  <div>
    <v-row v-if="!items">
      <v-col cols="12" xl="2" lg="3" md="3" sm="4" xs="6" v-for="i in 12" :key="i">
        ...
        <v-card elevation="0">
          <v-card-text class="pa-0 grey areaRecurso">
            ...
            <div class="commentchip" v-if="key.comentarios">
              {{ ncomentarios(key.comentarios) }}
              <v-icon class="melon--text icono">mdi-chat</v-icon>
            </div>

            <div class="favoritosicon" @click="addfavoritos(key)">
              <v-icon class="melon--text iconofav">{{
                key.favoritos && key.favoritos.length > 0
                  ? key.favoritos.indexOf(userdatacollect.uid) > 0
                    ? "mdi-cards-heart"
