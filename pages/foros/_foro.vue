<template>
  <v-card>
    <v-card-text>
      ...
      <b
        v-html="$moment(forodata.fechapub.seconds * 1002).format('YYYY/MM/DD')"
      ></b>
      <h3>Categoría: {{ forodata.categoria }}</h3>
      <p v-html="forodata.cuerpo"></p>
      <br />
      <div v-if="is_login && disable">
...
export default {
  async asyncData(ctx) {
    return await ctx.app.$fire.firestore
      .collection("foro")
      .doc(ctx.params.foro)
      .get()
      .then((datos) => {
    ...
    guardarComentario() {
      if (this.perstart) {
        var payload = {
          ...
          nombre:
            this.userdatacollect.nombre + " " + this.userdatacollect.apellido,
          validado: false,
        };
